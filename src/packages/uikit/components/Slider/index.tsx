...
const normalizeValue = (props: SliderProps, value?: number | Array<number>): Array<number> => {
...
export class Slider extends PureComponent<SliderProps, SliderState> {
        ...
        this.state = {
            ...
            values: updateValues({
                values: normalizeValue(
                    this.props,
                    this.props.value instanceof Animated.Value ? this.props.value.__getValue() : this.props.value
                ),
            }),
        };
    }
    ...
    _getRawValues(values: Array<Animated.Value> | Array<Animated.AnimatedInterpolation>) {
        return values.map((value) => value.__getValue());
    }

    _handleStartShouldSetPanResponder = (e: any): /* gestureState: GestureState */ boolean => this._thumbHitTest(e); // Should we become active when the user presses down on the thumb?

    _handleMoveShouldSetPanResponder(): /* e, gestureState: GestureState */ boolean {
    ...
    _handleMeasure = (name: '_containerSize' | '_trackSize' | '_thumbSize', e: LayoutChangeEvent) => {
    ...
    _getCurrentValue = (thumbIndex: number = 0) => this.state.values[thumbIndex].__getValue();

    _setCurrentValue = (value: number, thumbIndex: number | null | undefined, callback?: () => void) => {
    ...
    _renderDebugThumbTouchRect = (thumbLeft: Animated.AnimatedInterpolation, index: number) => {
    ...
    _renderThumbImage = (thumbIndex: number = 0) => {
        ...
        return (
            <Image source={(Array.isArray(thumbImage) ? thumbImage[thumbIndex] : thumbImage) as ImageSourcePropType} />
        );
    };
                ...
                {renderAboveThumbComponent && (
                    <View style={styles.aboveThumbComponentsContainer}>
                        {interpolatedThumbValues.map((interpolationValue, i) => {
                            ...
                            const value =
                                animatedValue instanceof Animated.Value ? animatedValue.__getValue() : animatedValue;
                            return (
                                <Animated.View
                ...
                {renderBelowThumbComponent && (
                    <View style={styles.belowThumbComponentsContainer}>
                        {interpolatedThumbValues.map((interpolationValue, i) => {
                            const animatedValue = values[i] || 0;
                            const value =
                                animatedValue instanceof Animated.Value ? animatedValue._getValue() : animatedValue;
                            return (
                                <Animated.View
